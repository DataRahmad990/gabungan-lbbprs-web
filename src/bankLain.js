...
function makeProcessor({ prefix, data, requireNamaCol, color, sheetRingkasan, title, namePrefix }) {
  return function (files, period, XLSX) {
    ...
    for (let br = 1; br <= 6; br++) {
      const code = String(br).padStart(3, "0");
    ...
    return { filename: `${namePrefix}_${period.periodeLabel}.xlsx`, data: out, summary };
  };
}
...
export const processAbp = makeProcessor({
  ...
  namePrefix: "GABUNGAN_ABP_LIABILITAS_BANK_LAIN_BPRS_SURIYAH",
});
...
export const processPenempatan = makeProcessor({
  ...
  namePrefix: "GABUNGAN_PENEMPATAN_BANK_LAIN_BPRS_SURIYAH",
});
